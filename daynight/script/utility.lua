...
do
    ...
    function FindTableItemByValue(tb, key, value)
        for i, val in ipairs(tb) do
            if tb[i][key] == value then
                return i
            end
        end
    end
...
do
    ...
    function oscillate(t, offset)
        local a = (GetTime() / (t or 1)) % 1
        a = a * math.pi
        return math.sin(a + (offset or 0))
        -- print(-0.5 * (math.cos(2 * math.pi * a)) + 0.5)
        -- return -0.5 * (math.cos(2 * math.pi * a)) + 0.5
...
do
    ...
    function sfnPadZeroes(n, pad)
        local format = "%0" .. tostring(pad or 2) .. "d"
